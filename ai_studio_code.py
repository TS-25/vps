...
DOCKER_IMAGES = {
    "ubuntu-22.04": {
        ...
        "display_name": "Ubuntu 22.04 (systemd)",
        "description": "Ubuntu 22.04 with tmate and systemctl support.",
        "ram": "6GB",
        "cpu": "2 cores"
    },
}
...
# Helper function to sanitize hostname
def sanitize_hostname(name: str) -> str:
    """
    Sanitizes a string to be a valid RFC 1123 hostname.
    - Converts to lowercase.
    - Replaces invalid characters with a hyphen.
    - Removes leading/trailing hyphens.
    - Ensures it's not empty and truncates to 63 characters.
    """
    hostname = name.lower()
    ...
    if not hostname:
        hostname = 'discord-vps'
    # Return the sanitized hostname, truncated to 63 characters
    return hostname[:63]
...
def get_container_info(container_id: str) -> Optional[Dict]:
    """
    Finds a container by its ID and returns its info, including the owner's user ID.
    """
    data = load_database()
    
    for user_id, containers in data.items():
        for container in containers:
            # Match partial or full container ID
...
async def create_server_task(interaction: discord.Interaction, image_name: str):
    ...
    try:
        ...
        # Step 2: Create container with systemd and hostname support
        embed.set_field_at(0, name="Status", value="🛠️ Creating container...", inline=False)
        await message.edit(embed=embed)
        
        # Sanitize Discord username for use as a hostname
        hostname = sanitize_hostname(interaction.user.name)
        
        try:
            # Parameters to enable systemd (systemctl) support
            container = client.containers.run(
                image_data['name'],
                command="/sbin/init",  # Start systemd as PID 1
                hostname=hostname,     # Set the container hostname
                detach=True,
                tty=True,
                privileged=True,       # Required for systemd to manage the system
                volumes={'/sys/fs/cgroup': {'bind': '/sys/fs/cgroup', 'mode': 'ro'}}, # Mount cgroup
                tmpfs={'/run': '', '/run/lock': ''}, # Provide tmpfs for systemd
                mem_limit='6g',
                cpu_quota=200000,
                cpu_shares=512,
        ...
        try:
            # Give systemd a moment to start up before running exec
...
async def manage_server(interaction: discord.Interaction, action: str, container_id: str):
    ...
    # Use the user_id from the fixed get_container_info function
    ...
    # Use the full container ID for docker commands
    full_container_id = container_info['container_id']
    
    try:
        container = client.containers.get(full_container_id)
        image_data = DOCKER_IMAGES.get(container_info['image'], {})
        
        if action == "start":
            container.start()
            status = "started"
            update_container_status(full_container_id, "running")
        elif action == "stop":
            container.stop()
            status = "stopped"
            update_container_status(full_container_id, "stopped")
        elif action == "restart":
            container.restart()
            status = "restarted"
            update_container_status(full_container_id, "running")
            ...
            container.stop()
            container.remove()
            remove_from_database(full_container_id)
            status = "removed"
        else:
        ...
        embed = discord.Embed(
            ...
            description=f"Instance `{full_container_id[:12]}` has been {status}.",
            color=0x00ff00
        )
        
        if action != "remove":
            stats = await get_container_stats(full_container_id)
            if stats:
                embed.add_field(
                    name="Resources",
        ...
        if action in ["start", "restart"]:
            ...
            try:
                ...
                await asyncio.sleep(3)
                exec_cmd = await asyncio.create_subprocess_exec(
                    "docker", "exec", full_container_id, "tmate", "-F",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                ssh_session_line = await capture_ssh_session_line(exec_cmd)
...
# --- The rest of your code remains unchanged. I'll include it for completeness. ---
# (Note: I've also updated functions like regen_ssh and info to use the fixed get_container_info logic)
...
async def regen_ssh_command(interaction: discord.Interaction, container_id: str):
    ...
    if not container_info:
        embed = discord.Embed(title="Instance Not Found", description="No instance found with that ID.", color=0xff0000)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    
    if container_info['user_id'] != user and interaction.user.id not in ADMIN_IDS:
        embed = discord.Embed(title="Permission Denied", description="You don't have permission to manage this instance.", color=0xff0000)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        data = load_database()
        for uid, containers in data.items():
            for c in containers:
                if c["container_id"] == full_container_id:
                    c["ssh_command"] = ssh_session_line
        save_database(data)
        
        image_data = DOCKER_IMAGES.get(container_info['image'], {})
        embed = discord.Embed(title=f"🔑 New SSH Session for {image_data.get('display_name', 'Instance')}", description=f"```{ssh_session_line}```", color=0x00ff00)
        embed.add_field(name="Instance ID", value=full_container_id[:12], inline=False)
        
        await interaction.user.send(embed=embed)
        await interaction.followup.send(embed=discord.Embed(description="New SSH session generated. Check your DMs!", color=0x00ff00))
    
    except Exception as e:
        embed = discord.Embed(title="Error Generating SSH Session", description=str(e), color=0xff0000)
        await interaction.followup.send(embed=embed)
...
async def show_instance_info(interaction: discord.Interaction, container_id: str):
    ...
    user = str(interaction.user.id)
    if container_info['user_id'] != user and interaction.user.id not in ADMIN_IDS:
        embed = discord.Embed(title="Permission Denied", description="You don't have permission to view this instance.", color=0xff0000)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        embed = discord.Embed(title=f"{image_data.get('display_name', 'Instance')} Details", color=0x3498db)
        embed.add_field(name="Instance ID", value=full_container_id[:12], inline=True)
        embed.add_field(name="Status", value=container.status.capitalize(), inline=True)
        embed.add_field(name="Created", value=datetime.datetime.fromisoformat(container_info['created_at']).strftime('%Y-%m-%d %H:%M'), inline=True)
        
        if stats:
            embed.add_field(name="CPU Usage", value=f"{stats['cpu_percent']}%", inline=True)
            embed.add_field(name="Memory Usage", value=f"{stats['memory_percent']}% ({stats['memory_usage']/1024/1024:.2f}MB/{stats['memory_limit']/1024/1024:.2f}MB)", inline=True)
        
        if container_info.get('ssh_command'):
            embed.add_field(name="SSH Access", value=f"```{container_info['ssh_command']}```", inline=False)
        
        view = View()
        if container.status == 'running':
            stop_button = Button(label="Stop", style=discord.ButtonStyle.red, emoji="⏹️", custom_id=f"stop_{full_container_id}")
            stop_button.callback = lambda i: manage_server(i, "stop", full_container_id)
            view.add_item(stop_button)
            
            restart_button = Button(label="Restart", style=discord.ButtonStyle.blurple, emoji="🔄", custom_id=f"restart_{full_container_id}")
            restart_button.callback = lambda i: manage_server(i, "restart", full_container_id)
            view.add_item(restart_button)
        else:
            start_button = Button(label="Start", style=discord.ButtonStyle.green, emoji="▶️", custom_id=f"start_{full_container_id}")
            start_button.callback = lambda i: manage_server(i, "start", full_container_id)
            view.add_item(start_button)
        
        ssh_button = Button(label="Regen SSH", style=discord.ButtonStyle.gray, emoji="🔑", custom_id=f"ssh_{full_container_id}")
        ssh_button.callback = lambda i: regen_ssh_command(i, full_container_id)
        view.add_item(ssh_button)
        
        remove_button = Button(label="Remove", style=discord.ButtonStyle.red, emoji="🗑️", custom_id=f"remove_{full_container_id}")
        remove_button.callback = lambda i: manage_server(i, "remove", full_container_id)
        view.add_item(remove_button)
        
        await interaction.followup.send(embed=embed, view=view)
    
    except docker.errors.NotFound:
        embed = discord.Embed(title="Instance Not Found", description="The container no longer exists.", color=0xff0000)
        await interaction.followup.send(embed=embed)
        remove_from_database(full_container_id)
    except Exception as e:
        embed = discord.Embed(title="Error Getting Instance Info", description=str(e), color=0xff0000)
        await interaction.followup.send(embed=embed)
...
async def deploy(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="🚀 Deploy a New Instance", description="Select an OS image from the dropdown below:", color=0x3498db)
...
async def list_instances(interaction: discord.Interaction):
    ...
    if not containers:
        embed = discord.Embed(title="No Instances Found", description="You don't have any instances yet. Use `/deploy` to create one!", color=0xff0000)
        await interaction.response.send_message(embed=embed)
        return
    
    embed = discord.Embed(title="Your Instances", description=f"You have {len(containers)}/{SERVER_LIMIT} instances", color=0x3498db)
    for container in containers:
        image_data = DOCKER_IMAGES.get(container['image'], {})
        status = container.get('status', 'unknown').capitalize()
        embed.add_field(name=f"{image_data.get('display_name', 'Instance')} ({container['container_id'][:12]})", value=f"Status: {status}\nCreated: {datetime.datetime.fromisoformat(container['created_at']).strftime('%Y-%m-%d')}", inline=True)
    await interaction.response.send_message(embed=embed)
...
async def stats(interaction: discord.Interaction):
    ...
    try:
        ...
        embed = discord.Embed(title="System Statistics", color=0x3498db)
        embed.add_field(name="CPU Usage", value=f"{cpu_percent}%", inline=True)
        embed.add_field(name="Memory Usage", value=f"{memory.percent}% ({memory.used/1024/1024:.0f}MB/{memory.total/1024/1024:.0f}MB)", inline=True)
        embed.add_field(name="Disk Usage", value=f"{disk.percent}% ({disk.used//(1024*1024*1024)}GB/{disk.total//(1024*1024*1024)}GB)", inline=True)
        embed.add_field(name="Docker Containers", value=f"{running_containers}/{total_containers} running", inline=True)
        await interaction.followup.send(embed=embed)
    except Exception as e:
        embed = discord.Embed(title="Error Getting Statistics", description=str(e), color=0xff0000)
        await interaction.followup.send(embed=embed)

@bot.tree.command(name="help", description="Show help information")
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(title="Instance Manager Help", description="Manage your Docker instances through Discord", color=0x3498db)
    embed.add_field(name="/deploy", value="Create a new instance with a graphical interface", inline=False)
    embed.add_field(name="/list", value="List all your instances", inline=False)
    embed.add_field(name="/info <id>", value="Get detailed information about an instance", inline=False)
    embed.add_field(name="/start <id>", value="Start a stopped instance", inline=False)
    embed.add_field(name="/stop <id>", value="Stop a running instance", inline=False)
    embed.add_field(name="/restart <id>", value="Restart an instance", inline=False)
    embed.add_field(name="/regen-ssh <id>", value="Generate new SSH credentials", inline=False)
    embed.add_field(name="/remove <id>", value="Permanently remove an instance", inline=False)
    embed.add_field(name="/stats", value="Show system resource usage", inline=False)
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="admin-list", description="[ADMIN] List all instances")
async def admin_list(interaction: discord.Interaction):
    if interaction.user.id not in ADMIN_IDS:
        embed = discord.Embed(title="Permission Denied", description="This command is for admins only.", color=0xff0000)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    
    data = load_database()
    total_instances = sum(len(containers) for containers in data.values())
    embed = discord.Embed(title="All Instances", description=f"There are {total_instances} instances in total", color=0x3498db)
    ...
    for user_id, containers in data.items():
        ...
        embed.add_field(name=username, value=f"{len(containers)} instances", inline=True)
    await interaction.response.send_message(embed=embed)
